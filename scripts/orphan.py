...
def main():
    ...
    try:
        ...
        if not args.no_update_vcs:
            with ControlEditor(path=control_path) as editor:
                ...
                result.salsa_user = args.salsa_user
        ...
        if os.environ.get('DEB_UPDATE_CHANGELOG', 'update') != 'leave':
            add_changelog_entry(
                local_tree,
        ...
        local_tree.commit(
            "Move package to QA team.", committer=os.environ.get('COMMITTER'),
    ...
    result.pushed = False
    if not args.no_update_vcs and not args.just_update_headers and result.new_vcs_url:
    ...
    if os.environ.get('SVP_API') == '1':
        with open(os.environ['SVP_RESULT'], 'w') as f:
            json.dump(f, {
                ...
                'context': {
                    'old_vcs_url': result.old_vcs_url,
                    'new_vcs_url': result.new_vcs_url,
                    'pushed': result.pushed,
                    'wnpp_bug': result.wnpp_bug})
