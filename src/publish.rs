...
mod tests {
    ...
    fn test_empty_proposal_detection() {
        ...
        let result = PublishBuilder::new(&local_branch, &main_branch, "test-branch", Mode::Propose)
            .publish(
        ...
        // Should fail with EmptyMergeProposal error
        match result {
            Err(Error::EmptyMergeProposal) => {
                // Expected error
            }
            _ => panic!("Expected EmptyMergeProposal error, got: {:?}", result),
        }
    }
