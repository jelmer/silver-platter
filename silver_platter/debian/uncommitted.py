...
def import_uncommitted(tree, subpath):
    ...
    with contextlib.ExitStack() as es:
        ...
        try:
            ...
            raise ChangerError(
                "no-apt-sources", "No sources configured in /etc/apt/sources.list"
            )
        ...
        for version in missing_versions:
            ...
            download_snapshot(
                package_name, version, output_dir, no_preparation=no_preparation
            )
...
class UncommittedChanger(DebianChanger):
    ...
    def make_changes(
        ...
        try:
            ...
            raise ChangerError("tree-version-not-in-archive-changelog", str(e))
        ...
        reporter.report_metadata(
            "tags", [(tag_name, str(version)) for (tag_name, version, revid) in ret]
        )
    ...
    def get_proposal_description(self, applied, description_format, existing_proposal):
        return "Import missing uploads: %s." % (", ".join([str(v) for t, v in applied]))
