...
def backport_package(local_tree, subpath, target_release, author=None):
    ...
    with ChangelogEditor(clp) as cl:
        ...
        changeblock_add_line(
            block,
            ["Backport to %s." % target_release] + [" +" + line for line in changes],
        )
...
class BackportChanger(DebianChanger):
    ...
    def make_changes(
        ...
        with tempfile.TemporaryDirectory() as td:
            builder = self.builder.replace("${LAST_VERSION}", str(since_version))
        ...
        branches = [("main", None, base_revision, local_tree.last_revision())]
